...
subBblId <- with(SubScrip, paste0(Book, Chapter, Verse))
Scripture$ID <- with(Scripture, paste0(Book, Chapter, Verse))
Comb_Subset <- Scripture %>% filter(ID %in% subBblId)
...
Bigram_Rel <- Bigram_Data %>% filter(n > 1) %>%
